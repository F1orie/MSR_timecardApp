...
export default function Header({ user, role, departmentName, userName }: { user: any, role?: string, departmentName?: string, userName?: string | null }) {
    return (
        <header className="flex justify-between items-center p-4 bg-slate-900 border-b border-slate-800 text-white">
            <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-medium text-gray-400 border-l border-gray-700 pl-4">
                    {departmentName ? `${departmentName}：` : ''}{userName || user?.email}
                </span>
            </div>
            <div className="flex items-center gap-4">
                {role === 'admin' && (
